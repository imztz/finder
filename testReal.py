...
def GetSolution(STEPRATIO, MODEL_FILE_CKPT):
    ...
    data_test_name = ['Crime','HI-II-14','Digg','Enron','Gnutella31','Epinions','Facebook','Youtube']
...
def EvaluateSolution(STEPRATIO, MODEL_FILE_CKPT, STRTEGYID):
    ...
    data_test_path = './data/real/'
    data_test_name = ['Crime', 'HI-II-14', 'Digg', 'Enron', 'Gnutella31', 'Epinions', 'Facebook', 'Youtube']
    save_dir = './results/real/StepRatio_%.4f/' % STEPRATIO
    ## begin computing...
    df = pd.DataFrame(np.arange(2 * len(data_test_name)).reshape((2, len(data_test_name))), index=['solution', 'time'],
                      columns=data_test_name)
...
def FindBestModel():
    csv_file_path = './models/Model_barabasi_albert/ModelVC_30_50.csv'
    df = pd.read_csv(csv_file_path, header=None)
    min_score_row = df[0].idxmin()
    x = (min_score_row - 1) * 300
    model_file_ckpt = f'nrange_30_50_iter_{x}.ckpt'
    return model_file_ckpt

def main():
    model_file_ckpt = FindBestModel()
